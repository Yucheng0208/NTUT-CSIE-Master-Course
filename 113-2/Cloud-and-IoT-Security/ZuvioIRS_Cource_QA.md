...
> **Answers:** 
> - Subnetworks：Divide the IP address range into smaller segments.（劃分 IP 範圍。）
> - Routes：Control how traffic flows within the network.（控制資料流向。）
> - Firewall Rules：Define rules to allow or deny network traffic.（設定流量允許或拒絕規則）。
> - VPN Gateways：Connect on-premises data centers to Google Cloud securely.（連接本地資料中心與雲端。）
